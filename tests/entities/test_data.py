...
@pytest.mark.parametrize('function_type, message', [
    pytest.param(
        'aggregation',
        'Invalid AggregationFunction uid=fake-uid',
        id='AggregationFunctionType'
    ),
    pytest.param(
        'group_by',
        'Invalid GroupByFunction uid=fake-uid',
        id='GroupByFunctionType'
    ),
])
